...
def connect():
    ...
    if data_type == 0:
        ...
        print('hihi3')
        # 中控确认，允许作业
        allow_work_flag = 1 if truck_id_confirm else 0 #车牌号正确，允许作业；否则allow_work_flag = 0 不允许作业
        allow_plc_work = 1 if truck_id_confirm else 0 #车牌号正确，启动PLC；否则allow_plc_work =0 停止启动
        ...
        if allow_work_flag == 1:
            ...
            engine = create_engine('sqlite:///example.db')  # 使用你的数据库连接  
            Session = sessionmaker(bind=engine)  
            session = Session() 
            user = session.query(Traffic).filter_by(truckid = truck_id, loadcurrent = truck_load, goodstype = goods_type).first()
            ...
            data_start_time = datetime.datetime.now() # 获取开始送料时的时间
            ...
            insert_truck_content(req_time,truck_load,load_current,truck_id,truck_weight_in,truck_weight_out,goods_type,store_id,loader_id,data_start_time,data_stop_time)#数据存储到数据库中
